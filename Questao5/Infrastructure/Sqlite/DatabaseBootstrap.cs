...
namespace Questao5.Infrastructure.Sqlite
{
    public class DatabaseBootstrap : IDatabaseBootstrap
    {
        ...
        public IEnumerable<T> ExecuteSelectQuery<T>(string sqlSelectQuery, object queryParams)
        {
            using var connection = new SqliteConnection(databaseConfig.Name);
            return connection.Query<T>(sqlSelectQuery, queryParams);
        }

        public IEnumerable<int> ExecuteQueries(List<KeyValuePair<string, object>> queries)
        {
            var queriesResults = new List<int>();
            using var connection = new SqliteConnection(databaseConfig.Name);
            connection.Open();
            ...
            try
            {
                ...
                {
                    queriesResults.Add(connection.Execute(query.Key, query.Value, dbTransaction));
                }
